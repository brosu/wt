...
import (
	...
	"github.com/timvw/wt/internal/color"
)
...
func formatStatusLineColor(st worktreeStatus, useColor bool) string {
	marker := " "
	if st.Current {
		marker = "*"
	}
	...
	// Apply colors
	if st.Current {
		marker = color.Colorize("*", color.Bold+";"+color.CyanRaw)
	}

	branch := color.Colorize(st.Branch, color.Bold)

	if st.Dirty {
		state = color.Colorize("dirty", color.Red)
	} else {
		state = color.Colorize("clean", color.Green)
	}

	if !st.HasUpstream {
		tracking = color.Colorize("no upstream", color.Dim)
	} else {
		aheadStr := fmt.Sprintf("↑%d", st.Ahead)
		behindStr := fmt.Sprintf("↓%d", st.Behind)
		if st.Ahead > 0 {
			aheadStr = color.Colorize(aheadStr, color.Yellow)
		}
		if st.Behind > 0 {
			behindStr = color.Colorize(behindStr, color.Yellow)
		}
...
func formatCIColor(ci string) string {
	switch ci {
	case "pass":
		return color.Colorize("✓ CI", color.Green)
	case "fail":
		return color.Colorize("✗ CI", color.Red)
	case "pending":
		return color.Colorize("● CI", color.Yellow)
	default:
		return color.Colorize(ci, color.Dim)
	}
}
